...
exports.postRegister = async (req, res, next) => {
  ...
  if (password !== confirmPassword) {
    ...
    if (count !== 0) {
      errors.push({ msg: 'Esse email ja está em uso!' });
      return res.render('Admin/register', { errors });
    } else {
...
exports.postInfoSettings = async (req, res, next) => {
  ...
  if (!(await bcrypt.compare(password, user.password))) {
    ...
    req.flash('success_msg', 'Informações atualizadas com sucesso!');
    return res.redirect('/admin/info_settings');
  }
};
...
exports.postAddStaff = async (req, res, next) => {
  ...
  const count = (await queryParamPromise(sql1, [email]))[0].count;
  if (count !== 0) {
    req.flash('error', 'Professor com esse email já existe!');
    ...
    if (contact.length > 11) {
      req.flash('error', 'Contato inválido!');
    ...
    req.flash('success_msg', 'Professor adicionado com sucesso');
...
exports.postRelevantStaff = async (req, res, next) => {
  ...
  if (department === 'None' && section !== '') {
    req.flash('error', 'Por favor selecione um departamento!');
    ...
    if (max_section !== null && section <= max_section) {
      ...
      req.flash('error', 'Seção não existe neste departamento!');
      res.redirect('/admin/getStaff');
    }
...
exports.postAddStudent = async (req, res, next) => {
  ...
  req.flash('success_msg', 'Estudadante adicionado com sucesso');
...
exports.getAllStudent = async (req, res, next) => {
  const sql = 'SELECT * from student where disabled = 0';
  const results = await zeroParamPromise(sql);
  res.render('Admin/Student/getStudent', {
    data: results,
    page_name: 'students',
...
exports.postStudentSettings = async (req, res, next) => {
  ...
  req.flash('success_msg', 'Estudante modificado com sucesso');
  res.redirect('/admin/getAllStudents');
};

// 3.5 Delete existing students
exports.getStudentDelete = async (req, res, next) => {
  const studentEmail = req.params.id;
  const sql1 = 'SELECT * FROM STUDENT WHERE email = ?';
  const studentData = await queryParamPromise(sql1, [studentEmail]);
  const address = studentData[0].s_address.split('-');
  ...
  res.render('Admin/Student/deleteStudent', {
    studentData: studentData,
    departments: departments,
    page_name: 'students',
  });
...
exports.postStudentDelete = async (req, res, next) => {
  ...
  const sql2 =
    'UPDATE STUDENT SET s_name = ?, gender = ?, dob = ?,email = ?, s_address = ?, contact = ?, password = ?, section = ?, dept_id = ?, disabled = ? WHERE email = ?';
  await queryParamPromise(sql2, [
    name,
    gender,
    dob,
  ...
  req.flash('error_msg', 'Estudante deletado com sucesso');
  res.redirect('/admin/getAllStudents');
};

// 3.6 Activate existing students
exports.getStudentActivate = async (req, res, next) => {
  const studentEmail = req.params.id;
  const sql1 = 'SELECT * FROM STUDENT WHERE email = ?';
  const studentData = await queryParamPromise(sql1, [studentEmail]);
  const address = studentData[0].s_address.split('-');
  ...
  res.render('Admin/Student/activateStudent', {
    studentData: studentData,
    departments: departments,
    page_name: 'students',
  });
...
exports.postStudentActivate = async (req, res, next) => {
  const {
    old_email,
    email,
    dob,
  ...
  req.flash('Success_msg', 'Estudante ativado com sucesso');
  res.redirect('/admin/getAllStudents');
};
...
exports.postAddClass = async (req, res, next) => {
  ...
  if (staffData.dept_id !== courseData.dept_id) {
    req.flash('error', 'O professor e o curso são de departamentos diferentes');
  ...
  if (section <= 0 || section > max_section) {
    req.flash('error', 'A sessão não existe');
    return res.redirect('/admin/addClass');
  }
...
exports.postClassSettings = async (req, res, next) => {
  ...
  req.flash('success_msg', 'Turma modificada com sucesso');
  res.redirect('/admin/getClass');
};
...
exports.postAddDept = async (req, res, next) => {
  ...
  if (results.length !== 0) {
    ...
    req.flash('success_msg', 'Departamento adicionado com sucesso!');
...
exports.postDeptSettings = async (req, res, next) => {
  ...
  req.flash('success_msg', 'Departamento modificado com sucesso');
  res.redirect('/admin/getDept');
};
...
exports.postCourseSettings = async (req, res, next) => {
  ...
  req.flash('success_msg', 'Cursos modificados com sucesso!');
